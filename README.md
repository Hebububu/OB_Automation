...
- read_xlsx로 읽어온 데이터를 바탕으로 순차적으로 관리 제품명 입력 기능
- read_xlsf로 읽어온 데이터를 바탕으로 순차적으로 관리 태그명 입력 기능
- read_xlsx로 읽어온 데이터와 db의 태그 비교 기능
 
# 생각을 잘못했다. 

판매 플랫폼이 너무 많고, 판매자 계정도 다양하게 있는데 기기만 출고 자동화 대상이다.
전체 상품의 DB를 가져오자니 너무 크고 방대한데다, 지금 자동화를 하려다 수작업을 더 많이 하고 있는 꼴임..

다른 로직을 생각해야한다..
사람이 판단하는 기준.. 이게 기기인지 아닌지를 코드로 바꾸는 생각을 하자..
